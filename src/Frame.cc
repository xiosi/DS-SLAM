...
void Frame::CalculEverything(cv::Mat &imRGB, const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imS)
{
    ...
    if (!T_M.empty() && flagprocess) //T_M: Outliers vector && people exists 如果場景中有人並且有异常點，則通過label進行進一步的運動檢查
    {
...
/**
 * 將原來的void Frame::ExtractORB(int flag, const cv::Mat &im)
 * (*mpORBextractorLeft)(im, cv::Mat(), mvKeys, mDescriptors);
 * 拆分爲計算特征和描述子兩部分 
 * mvKeysTemp is used for CheckMovingKeyPoints
 */
void Frame::ExtractORBKeyPoints(int flag, const cv::Mat &imgray)
{
    if (flag == 0)
    {
        (*mpORBextractorLeft)(imgray, cv::Mat(), mvKeysTemp); //2: _mask, 3:_keypoints
    }
    else
        (*mpORBextractorRight)(imgray, cv::Mat(), mvKeysTemp);
}
/**
 * outliers has aready removed from mvKeysTemp
 */
