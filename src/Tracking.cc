...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, Map *pMap, boost::shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase *pKFDB, const string &strSettingPath, const int sensor) : mSensor(sensor), mbOnlyTracking(false), mbVO(false), mbNewSegImgFlag(false), mpORBVocabulary(pVoc), mpPointCloudMapping(pPointCloud), mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer *>(NULL)), mpSystem(pSys), mpMap(pMap), mnLastRelocFrameId(0)
{
